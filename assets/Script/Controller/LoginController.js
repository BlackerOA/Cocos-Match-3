...
cc.Class({
  ...
  getHowToPlayText() {
    return `這是一款經典三消遊戲，玩家要在限制的時間內思考出最佳的消除策略，並在有限的步數中盡可能地取得更多的金幣。透過消除更多動物、取得高combo以及消除特殊方塊組合來獲得更多的金幣來達成階段目標，並一路闖關到第三關來衝刺最高分，最後在排行榜上打敗別人吧!
  ...
  【遊戲階段系統】
  遊戲分為三個階段，每個階段有不同的目標分數、步數限制和特殊方塊掉落機率：
  ...
  註：
  - 左下角分數顯示會根據達標狀態變色（白色→黃色達標→紅色警告）
  - 階段轉換時會顯示過場提示框，持續 2 秒
  - 每個階段的步數會重置，但分數是累計的
  ...
  ● 單次消除:
    ...
    - 18~20 顆：+120 金幣
    - 21+ 顆：+150 金幣
  ● Combo 額外獎勵：3 × (combo²) 金幣
    - 例如：combo 5 = 3 × 25 = 75 金幣
  ● 特殊組合額外獎勵：
    - 直/橫 + 直/橫：+250 金幣
    - 直/橫 + 爆炸：+400 金幣
    - 直/橫 + 鳥：+600 金幣
